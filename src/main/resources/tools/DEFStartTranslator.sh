...
then
    ...
    if [[ "$version" > "1.8" ]]; then
        ...
        if [ "$(uname)" == "Darwin" ]; then
          DEF_JVM_OPTS="-Djava.library.path=./lib/native/mac-x86_64 -Dorg.apache.activemq.SERIALIZABLE_PACKAGES=* -jar ./lib/DahuDEFServer.jar -cdir ./config -c DEFConfig_Translator_QBE.json"
        else
          DEF_JVM_OPTS="-Djava.library.path=./lib/native/linux-x86_64 -Dorg.apache.activemq.SERIALIZABLE_PACKAGES=* -jar ./lib/DahuDEFServer.jar -cdir ./config -c DEFConfig_Translator_QBE.json"
